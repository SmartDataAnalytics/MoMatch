...
class ClassSimilarity {
  ...
  def GetMultilingualClassSimilarity(O1Translated: RDD[(String, String)], O2: RDD[String]): RDD[(String, String, String, Double)]={
...
//    tt.foreach(println(_))
  ...
  def GetMonolingualClassSimilarity(O1: RDD[String], O2: RDD[String]): RDD[(String, String, Double)]={
    val gS = new GetSimilarity()
    val t = O1.cartesian(O2)
    val tt = t.map(x => (x._1, x._2, gS.getSimilarity(x._1.toLowerCase,x._2.toLowerCase))).filter(y=>y._3>0.9)
      // tt.foreach(println(_))
    tt.distinct(2)
  }
