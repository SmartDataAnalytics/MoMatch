...
object Try {
  def main(args: Array[String]): Unit = {
    ...
    println(LevenshteinMetric.compare("topic", "topic").head) // 2
    println("Levenshtein.distance "+ Levenshtein.distance("example", "samples"))  // 3
    println("Levenshtein.score "+ Levenshtein.score("example", "samples"))// 0.5714285714285714
    // Normalized Levenshtein
    val l = new NormalizedLevenshtein()
    println("Normalized Levenshtein = "+l.distance("example", "samples"))
    //      Hamming Metric:
    println("Hamming Metric:")
    println(HammingMetric.compare("toned", "roses").head) // 3
    println(HammingMetric.compare("alex", "john").head) // 4
    println("Normalized Hamming = "+Hamming.score("alex", "john"))//0
    // Needleman Wunsch
...
//    Simple Ratio
//    val ratio: Int = FuzzySearch.ratio("mysmilarstring","myawfullysimilarstirng") //72
//    val ratio = FuzzySearch.ratio("Robert","Rupert") //67
    val ratio = FuzzySearch.ratio("Sue","sue") //67
    println("Ratio = " + ratio/100.toDouble)

    //    Partial Ratio
//    val partialRation: Int = FuzzySearch.partialRatio("similar", "somewhresimlrbetweenthisstring") //71
    val partialRation: Int = FuzzySearch.partialRatio("conference", "activity take place after conference")
//    val partialRation: Int = FuzzySearch.partialRatio("example", "samples")
    ...
    println("Partial token sort = "+partailTokenSort/100.toDouble)
    //    Token Sort
//    val tokenSort = FuzzySearch.tokenSortRatio("order words out of","  words out of order") //100
    ...
    println("Token sort = "+tokenSort/100.toDouble)
